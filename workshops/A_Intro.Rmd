---
title: "Spatial Visualisation | Introduction"
...
```{r,echo=F}
if(internet){
  link1 <- "https://en.wikipedia.org/wiki/Fra_Mauro_map#/media/File:FraMauroChineseShip.jpg"  
}else{
  link1 <- paste(graph.path,"FraMauroChineseShip.jpg",sep="")
}
```
![pic](`r link1 `)
...
```{r,echo=F}
link2 <- paste(graph.path,"Zensus_Mannheim2.png",sep="")
```

![pic](`r link2 `)

Source: [Census Atlas](https://atlas.zensus2011.de/)
...
if(internet){
  ...
  link1 <- paste(graph.path,"Logo_OSM.png",sep="")
}
```
![pic21](`r link1 `)
...
if(internet){
  ...
  link1 <- paste(graph.path,"400px-Overpass_API_logo.svg.png",sep="")
}
```
![pic21](`r link1 `)
