...
```{r}
unemp$colorSteps <- cut(unemp$unemp, c(0, 2, 4, 6, 8,10, 100))
colorsmatch <- unemp$colorSteps[match(county.fips$fips, unemp$fips)]

map("county", col = colors[colorsmatch], fill = TRUE)
```
