...
abcm <- xmlParse(paste("http://open.mapquestapi.com/xapi/api/0.6/node%5Bamenity=pub%5D%5Bbbox=",
                       bbox4,"%5D",sep=""))
```
...
bb_SW <- create_bbox(location="Schwetzingen",box.value=0.0075)
xapi_x <- xmlParse(paste("http://open.mapquestapi.com/xapi/api/0.6/node%5B",object_x,
                         "%5D%5Bbbox=",bb_SW,"%5D",sep=""))
```


```{r,eval=F}
object_x1 <- "school"
save(xapi_x,file=paste("xapi_HD_",location,"_",object_x1,".RData",sep=""))
```


## Get results for a bigger area

bus station does not work so good
